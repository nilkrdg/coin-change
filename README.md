...
When the user inserts coins into the machine, the user coins are saved in an array in memory. The reason to keep user coins seperately is to return them back in case of a request results with an error. Not enough change, not enough user coins to buy etc.<br><br>
